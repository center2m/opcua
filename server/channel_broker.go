...
func (c *channelBroker) RegisterConn(ctx context.Context, conn *uacp.Conn, localCert []byte, localKey *rsa.PrivateKey) error {
	...
	c.secureChannelID++
	c.secureTokenID++
	cfg.SecureChannelID = c.secureChannelID
	cfg.SecurityTokenID = c.secureTokenID
	c.mu.Unlock()

	sc, err := uasc.NewSecureChannel("", conn, cfg)
	if err != nil {
...
outer:
	for {
		...
		default:
			msg := sc.Receive(ctx)
			if msg.Err == io.EOF {
				debug.Printf("Secure Channel %d closed", cfg.SecureChannelID)
				break outer
			}
			c.msgChan <- msg
		}
	...
	c.mu.Lock()
	delete(c.s, cfg.SecureChannelID)
	c.mu.Unlock()
	c.wg.Done()
