...
func (s *Server) Start(ctx context.Context) error {
	...
	s.l, err = uacp.Listen(s.addr, nil)
	if err != nil {
		return err
	}
	log.Printf("Started listening on %s", s.l.Addr())
...
func (s *Server) generateEndpoints() error {
	...
	for _, sec := range s.cfg.enabledSec {
		...
		ep := &ua.EndpointDescription{
			EndpointURL:   s.addr, // todo: be able to listen on multiple adapters
